...
`gold-dust-gateway` is a small Rust project that acts as a local HTTP CONNECT proxy, a routing/policy layer, and a lightweight dashboard for controlling proxy mode.

It’s designed as a portfolio piece showing:

* A local HTTP proxy (`dispatcher`) that can route traffic via Tor or direct based on a flag file.
* A CLI binary (`gold-dust-gateway`) that simulates backend routing decisions and can query Krypton entropy health.
* A web dashboard (`dashboard`) that shows and toggles the proxy mode (Tor/Direct).
